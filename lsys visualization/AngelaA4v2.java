...
public class AngelaA4v2 extends JComponent {
	...
	static private float segmentLength = 40;
	...
	public void paint(Graphics g) {
		...
		// twoDG.setPaint(new Color(79, 79, 79));// gray
		...
		for (int i = 0; i < storageIndex; i++) {
			...
			if (ellipseType == 1) {
				twoDG.setPaint(new Color(255, 255, 0));// yellow
			} else if (ellipseType == 2) {

				twoDG.setPaint(new Color(32, 178, 170));// green blue
			} else if (ellipseType == 3) {
				twoDG.setPaint(new Color(255, 175, 0));// orange
			} else {
			...
			segment.setLine(paintData[i][1], paintData[i][2], paintData[i][3],
					paintData[i][4]);
			twoDG.draw(segment);
			twoDG.setPaint(new Color(127, 255, 0));
			segment.setLine(paintData[i][1] - 4, paintData[i][2] - 4,
					paintData[i][3] - 4, paintData[i][4] - 4);
			twoDG.draw(segment);
			twoDG.setPaint(Color.gray);
			segment.setLine(paintData[i][1] - 10, paintData[i][2] - 10,
					paintData[i][3] - 10, paintData[i][4] - 10);
			twoDG.draw(segment);
		}
	...
	public static void getPaintData() {
		...
		System.out.println("");
		System.out.println("");
		System.out.println("printData contents for storageIndex: "
				+ storageIndex);
		for (int i = 0; i < storageIndex; i++) {
	...
	public static void calculateSegmentData(int start, int end) {
		...
		while (index < end) {
			...
			if (adajacent > 0.5 || adajacent < -0.5) {
				// System.out.print("paintData[index][1]: " +
				// paintData[index][1] +" + adajacent: "+ adajacent);
			...
			if (opposite > 0.5 || opposite < -0.5) {
				// System.out.print("paintData[index][2]: " +
				// paintData[index][2] +" + opposite: "+ opposite);
